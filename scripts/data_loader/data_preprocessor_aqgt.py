...
class DataPreprocessor:
    def __init__(self, clip_lmdb_dir, out_lmdb_dir, n_poses, subdivision_stride,
        ...
        self.spectrogram_sample_length = utils.data_utils_aqgt.calc_spectrogram_length_from_motion_length(self.n_poses, self.skeleton_resampling_fps)
    ...
    def _sample_from_clip(self, vid, clip):
        ...
        # skeleton resampling
        clip_skeleton = utils.data_utils_aqgt.resample_pose_seq(clip_skeleton, clip_e_t - clip_s_t, self.skeleton_resampling_fps)
        ...
        if len(sample_skeletons_list) > 0:
            with self.dst_lmdb_env.begin(write=True) as txn:
                for words, poses, audio, spectrogram, aux in zip(sample_words_list, sample_skeletons_list,
                    ...
                    dir_vec = utils.data_utils_aqgt.convert_pose_seq_to_dir_vec(poses)
