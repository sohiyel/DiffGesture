...
from data_loader.lmdb_data_loader_aqgt import SpeechMotionDataset, default_collate_fn
from model.embedding_space_evaluator import EmbeddingSpaceEvaluator
from utils.average_meter import AverageMeter
from utils.data_utils_aqgt import convert_dir_vec_to_pose, convert_pose_seq_to_dir_vec, resample_pose_seq, dir_vec_pairs
...
def evaluate_testset(test_data_loader, diffusion, embed_space_evaluator, args, pose_dim):
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            out_dir_vec_bc = out_dir_vec + torch.tensor(args.mean_dir_vec).unsqueeze(0).unsqueeze(0).cuda()
...
def main(mode, checkpoint_path):
    ...
    args.mean_pose, args.mean_dir_vec = pickle.load(open("means.p", "rb"))
    args.mean_pose = np.reshape(args.mean_pose, (159,))
    args.mean_dir_vec = np.reshape(args.mean_dir_vec, (159,))

    # load lang_model
    vocab_cache_path = os.path.join('data/dataset', 'vocab_cache.pkl')
    ...
    def load_dataset(path):
        dataset = SpeechMotionDataset(path,
                                      ...
                                      mean_dir_vec=args.mean_dir_vec
                                      )
        print(len(dataset))
        return dataset
...
if __name__ == '__main__':
    ...
    ckpt_path = 'output/train_diffusion_aqgt/pose_diffusion_checkpoint_499.bin'
